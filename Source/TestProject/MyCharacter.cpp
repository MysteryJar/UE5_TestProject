...
AMyCharacter::AMyCharacter()
{
	...
	GetMesh()->SetCollisionProfileName(TEXT("MyCharacter"));
...
void AMyCharacter::AttackCheck()
{
	...
	bool bResult = GetWorld()->SweepSingleByChannel( // 충돌 채널 결과 값 저장
		OUT HitResult,
		GetActorLocation(),
	...
	FVector Vec = GetActorForwardVector() * AttackRange; // 드로우 디버그 캡슐 인자들
	FVector Center = GetActorLocation() + Vec * 0.5f;
	float HalfHeight = AttackRange * 0.5f + AttackRadius;
	FQuat Rotation = FRotationMatrix::MakeFromZ(Vec).ToQuat();
	FColor DrawColor;
	if (bResult)	// 충돌 시 녹색 미충돌 시 적색
	...
	if (bResult && HitResult.GetActor()) {
		UE_LOG(LogTemp, Log, TEXT("Hit Actor : %s"), HitResult.GetActor());
	}
}
